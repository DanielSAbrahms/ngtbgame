...
def switchBoard(newboard):
    global background
    global mapMask
    os.chdir(os.path.expanduser('~') + "/Google Drive/PyGame Games/NGTB Game/Board" + str(newboard))
...
walk_cycle_speed = 25 #Lower is Faster
walking_counter = 0
animation_counter = 0
fullscreen = False
is_walking = False
...
while True:
    if player.rect.x > 730 and player.rect.y < 230 and player.rect.y > 170:
        switchBoard(2)
        player.rect.x = 0
    ...
    if is_walking == False and animation_delay > 60*animation_delay_time:
        if player_direction == DOWN: #Idle animation for player facing down ------------------------------
            if animation_counter > 59:
                animation_counter = 0
            image = animation(60,animation_counter,"DownIdleAnimation_CoinFlip")
            image = pygame.transform.scale(image,(90,100))
            player.image = image
        if player_direction == LEFT: #Idle animation for player facing left ------------------------------
            if animation_counter > 29:
                animation_counter = 0
            image = animation(30,animation_counter,"LeftIdleAnimation_Dance")
            image = pygame.transform.scale(image,(100,100))
            player.image = image
        if player_direction == RIGHT: #Idle animation for player facing right ------------------------------
            if animation_counter > 119:
                animation_counter = 0
            image = animation(120,animation_counter,"RightIdleAnimation_PhoneLook")
            image = pygame.transform.scale(image,(100,100))
            player.image = image
        animation_counter+=1
